...
namespace AtpCore\Api\PostNL;

use AtpCore\Api\PostNL\Response\AddressResult;
use AtpCore\Error;
...
class AdresApi
{
    ...
    public function getByPostalCodeHouseNumber($postalCode, $houseNumber)
    {
        try {
            // Get address-data
            $params = ["houseNumber"=>$houseNumber, "postalCode"=>$postalCode];
            if ($this->debug) $this->log("request", "lookup/address", json_encode($params));
            $result = $this->client->get("lookup/address", ["query"=>$params]);
            ...
            if ($this->debug) $this->log("response", "lookup/address", json_encode($response));
    ...
    private function mapAddressResponse($response, $failOnUndefinedProperty = true)
    {
        try {
            ...
            if (stristr($e->getMessage(), "JSON property") && stristr($e->getMessage(), "does not exist in object of type AtpCore\Api\PostNL\Response\Address")) {
